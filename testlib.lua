...
function module:win(config)
	...
	-- Theme = "somename" loads themes/somename.lua from ThemesFolder (a raw.githubusercontent
	-- folder URL you host yourself). Theme = "Custom" (or omitted) skips this entirely and
	-- relies only on ThemeOverrides below, which still applies on top either way.
	local themeName = config.Theme
	if type(themeName) == "string" and themeName ~= "" and themeName:lower() ~= "custom" then
		local folder = config.ThemesFolder
		if type(folder) == "string" and folder ~= "" then
			if not folder:match("/$") then
				folder = folder .. "/"
			end
			local ok, result = pcall(function()
				local src = game:HttpGet(folder .. themeName .. ".lua")
			...
			if ok and type(result) == "table" then
				...
				warn("VoidLib | Could not load theme '" .. tostring(themeName) .. "': " .. tostring(result))
			end
		else
			warn(
				"VoidLib | Theme = '"
					.. tostring(themeName)
					.. "' was set, but ThemesFolder is missing - falling back to the default theme."
			)
		end
	end
	...
	if config.KeySystem then
		...
		if type(keySettings.GetKey) == "table" then
			...
			if type(keySettings.GetKey[2]) == "table" then
				getKeyOptions = keySettings.GetKey[2]
			end
		end
		...
		if not keyPassed then
			...
			local keyFrame = create(
				"Frame",
				{
					Name = "KeySystemOverlay",
					Parent = screenGui,
					Size = UDim2.new(0, 350, 0, frameHeight),
					Position = UDim2.new(0.5, -175, 0.5, -(frameHeight / 2)),
					BorderSizePixel = 0,
					ZIndex = 20,
			...
			local dragHandle =
				create("Frame", { Parent = keyFrame, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 60) })
			...
			closeBtn.MouseEnter:Connect(function()
				ts:Create(closeBtn, TweenInfo.new(0.15), { TextColor3 = Color3.fromRGB(255, 80, 80) }):Play()
			end)
			...
			local nextY = 110
			if hasGetKey then
				local copyBtn = create(
					"TextButton",
					{
						Parent = keyFrame,
						Position = UDim2.new(0, 20, 0, nextY),
						Size = UDim2.new(1, -44, 0, 32),
						Text = copyOpts.Text or "Kopieren",
						TextSize = 12,
						AutoButtonColor = false,
					}
				)
			...
			local inputBg = create(
				"Frame",
				{ Parent = keyFrame, Position = UDim2.new(0, 20, 0, nextY), Size = UDim2.new(1, -44, 0, 36) }
			)
			reg(inputBg, "BackgroundColor3", "ElementBg")
			create("UICorner", { Parent = inputBg, CornerRadius = theme.ElementRadius })

			local keyInput = create(
				"TextBox",
				{
					Parent = inputBg,
					BackgroundTransparency = 1,
			...
			local checkBtn = create(
				"TextButton",
				{
					Parent = keyFrame,
					Position = UDim2.new(0, 20, 0, nextY + 44),
			...
			checkBtn.MouseButton1Click:Connect(function()
				...
				if match then
					...
					ts:Create(
						...
						{
							BackgroundTransparency = 1,
							Size = UDim2.new(0, 370, 0, frameHeight + 20),
							Position = UDim2.new(0.5, -185, 0.5, -(frameHeight + 20) / 2),
						}
					):Play()
					...
					task.delay(2, function()
						if noteLbl and noteLbl.Parent then
							noteLbl.TextColor3 = theme.SubText
						end
					end)
				end
			end)
			while not keyPassed and not keyClosed do
				task.wait(0.1)
			end
		end
	else
	...
	if config.ToggleUIKeybind then
		...
		elseif type(config.ToggleUIKeybind) == "string" then
			local ok, enumKey = pcall(function()
				return Enum.KeyCode[config.ToggleUIKeybind]
			end)
	...
	local function makeTopbarBtn(symbol)
		...
		btn.MouseEnter:Connect(function()
			ts:Create(
				...
				{ BackgroundTransparency = theme.ElementHoverTransparency, TextColor3 = theme.Accent }
			):Play()
		end)
	...
	if tabsPosition == "Top" then
		tabBar = create(
			"ScrollingFrame",
			{
				...
				BorderSizePixel = 0,
				BackgroundTransparency = theme.PanelTransparency,
				Size = UDim2.new(1, 0, 0, tabBarSize),
		...
		sectionsHolder = create(
			"Frame",
			{
				Name = "sectionsholder",
				Parent = body,
				BackgroundTransparency = 1,
				Position = UDim2.new(0, 0, 0, tabBarSize),
				Size = UDim2.new(1, 0, 1, -tabBarSize),
				ClipsDescendants = true,
			}
		)
	else
		...
		create(
			"UIPadding",
			{ Parent = tabBar, PaddingTop = UDim.new(0, 8), PaddingLeft = UDim.new(0, 8), PaddingRight = UDim.new(0, 8) }
		)
	...
	local function setSelectedTab(btn, section)
		...
		ts:Create(
			btn,
			TweenInfo.new(0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
			{ BackgroundTransparency = theme.ElementHoverTransparency }
		):Play()
		if indicator then
			ts:Create(
				indicator,
		...
		if glow then
			ts:Create(
				glow,
				TweenInfo.new(0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
				{ Transparency = theme.StrokeHoverTransparency }
			):Play()
		end
	...
	function sections:tab(title, icon)
		...
		local label = create(
			"TextLabel",
			{
				...
				Text = title,
				TextSize = 13,
				TextXAlignment = tabsPosition == "Top" and Enum.TextXAlignment.Center or Enum.TextXAlignment.Left,
			}
		)
		reg(label, "TextColor3", "Text")
		reg(label, "Font", "Font")

		local tabIconLbl = create(
			"ImageLabel",
			{
				Parent = btn,
				BackgroundTransparency = 1,
		...
		function contents:label(text, icon)
			text = checkText(text)
			local holder =
				create("Frame", { Parent = section, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 20) })

			local iconLbl = create(
				"ImageLabel",
				{
					Parent = holder,
					BackgroundTransparency = 1,
			...
			function labelObj:Set(newTitle, newIcon, newColor, ignoreTheme)
				if newTitle ~= nil then
					textLbl.Text = checkText(newTitle)
				end
				if newIcon ~= nil then
					applyIcon(newIcon)
				end
		...
		function contents:toggle(text, id, default, cb, opts)
			text = checkText(text)
			if type(id) == "boolean" or type(id) == "nil" then
				cb = default
				default = id
				id = text
			end
			...
			if savedData[id] ~= nil then
				toggled = savedData[id]
			end
			...
			local trackGlow = create(
				"UIStroke",
				{ Parent = track, Thickness = 1, Transparency = 0.6, ApplyStrokeMode = Enum.ApplyStrokeMode.Border }
			)
		...
		function contents:textbox(text, id, default, cb, opts)
			text = checkText(text)
			if type(id) == "string" and type(default) == "string" and type(cb) == "nil" then
				id = text
			end
			...
			if savedData[id] ~= nil then
				currentText = tostring(savedData[id])
			end
		...
		function contents:slider(text, id, min, max, default, cb, opts)
			...
			if savedData[id] ~= nil then
				valStart = tonumber(savedData[id]) or default
			end
		...
		function contents:dropdown(text, id, list, default, cb, opts)
			...
			local container = create(
				...
				{
					Parent = holder,
					Position = UDim2.new(0, 6, 0, theme.ElementHeight),
				...
				for _, val in ipairs(list) do
					...
					local opt = create(
						"TextButton",
						{
							...
							BackgroundTransparency = 0,
							BackgroundColor3 = theme.ElementHoverBg,
							Text = optionStr,
							TextSize = 12,
							AutoButtonColor = false,
						}
			...
			trigger.MouseButton1Click:Connect(function()
				...
				ts:Create(
					...
					TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
					{
						Size = dropdownOpen and UDim2.new(1, 0, 0, theme.ElementHeight + targetContainerHeight + 6)
							or UDim2.new(1, 0, 0, theme.ElementHeight),
					}
				):Play()
				ts:Create(
					container,
					TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
					{ Size = dropdownOpen and UDim2.new(1, -12, 0, targetContainerHeight) or UDim2.new(1, -12, 0, 0) }
				):Play()
				indicator.Text = dropdownOpen and "^" or "V"
			end)

			return {
				Refresh = function(_, nl, nd)
					list = nl or {}
					if nd then
						currentSelected = nd
						selectedLbl.Text = tostring(nd)
					end
					updateOptions()
				end,
				Get = function(_)
		...
		function contents:keybind(text, id, default, cb, opts)
			...
			if savedData[id] ~= nil then
				currentKey = tostring(savedData[id])
			end
			...
			if holdToInteract then
				ui.InputEnded:Connect(function(input)
					if
						...
						and input.KeyCode.Name == currentKey
					then
						holding = false
						if cb then
							pcall(cb, false)
		...
		function contents:CreateParagraph(config)
			...
			function paragraphObj:Set(newConfig)
				...
				if newConfig.Title ~= nil then
					titleLbl.Text = checkText(newConfig.Title)
				end
				if newConfig.Content ~= nil then
					contentLbl.Text = checkText(newConfig.Content)
				end
			end
		...
		function contents:CreateColorPicker(config)
			...
			if savedData[id] ~= nil and type(savedData[id]) == "table" then
				...
				pcall(function()
					currentColor = Color3.fromRGB(c[1] or 255, c[2] or 255, c[3] or 255)
				end)
			end
			...
			local lbl = create(
				"TextLabel",
				{
					...
					BackgroundTransparency = 1,
					Position = UDim2.new(0, 12, 0, 0),
					Size = UDim2.new(0.6, -12, 0, theme.ElementHeight),
					Text = cName,
					TextSize = 13,
					TextXAlignment = Enum.TextXAlignment.Left,
				}
			...
			local swatch = create(
				"TextButton",
				{
					Parent = holder,
					AnchorPoint = Vector2.new(1, 0.5),
					Position = UDim2.new(1, -12, 0, theme.ElementHeight / 2),
			...
			local panel = create(
				...
				{
					Parent = holder,
					Position = UDim2.new(0, 8, 0, theme.ElementHeight + 4),
			...
			for _, chDef in ipairs(channels) do
				local row =
					create("Frame", { Parent = panel, Size = UDim2.new(1, 0, 0, 24), BackgroundTransparency = 1 })
				local chLbl = create(
					"TextLabel",
					{
						Parent = row,
			...
			swatch.MouseButton1Click:Connect(function()
				...
				ts:Create(
					holder,
					TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
					{ Size = UDim2.new(1, 0, 0, theme.ElementHeight + (expanded and (panelHeight + 12) or 0)) }
				):Play()
				ts:Create(
					panel,
					TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
		...
		function contents:CreateDivider()
			local holder =
				create("Frame", { Parent = section, Size = UDim2.new(1, 0, 0, 9), BackgroundTransparency = 1 })
			local line = create(
				"Frame",
				{
					Parent = holder,
	...
	task.spawn(function()
		while loadingFrame and loadingFrame.Parent do
			local pulseOut = ts:Create(
				loadLogo,
				TweenInfo.new(1.1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
				{ Size = UDim2.new(0, 116, 0, 116), Position = UDim2.new(0.5, 0, 0, 32) }
			)
			local pulseIn = ts:Create(
				loadLogo,
				TweenInfo.new(1.1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
				{ Size = UDim2.new(0, 110, 0, 110), Position = UDim2.new(0.5, 0, 0, 35) }
			)
			pulseOut:Play()
	...
	task.spawn(function()
		local fillTween = ts:Create(
			barFill,
			TweenInfo.new(2.2, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out),
			{ Size = UDim2.new(1, 0, 1, 0) }
		)
		fillTween:Play()
		...
		local fadeTween = ts:Create(
			...
			{
				BackgroundTransparency = 1,
				Size = windowSize + UDim2.new(0, 40, 0, 40),
				Position = windowPosition - UDim2.new(0, 20, 0, 20),
			}
		)
